...
Resource* GeometryResource::vertColorClone()
{
	GeometryResource* res = new GeometryResource("", _flags);
	*res = *this;
	RenderDeviceInterface* rdi = Modules::renderer().getRenderDevice();

	// additionnal vertex data
	// xyz -> rgb
	// w -> shadow min (fix issues in shadows - holes due to bias and stuff)
	res->_vertColorData = new Vec4f[_vertCount];
	for (uint i = 0; i < _vertCount; i++) {
		res->_vertColorData[i].x = 0.0f; // r
		res->_vertColorData[i].y = 0.0f; // g
		res->_vertColorData[i].z = 0.0f; // b
		res->_vertColorData[i].w = 1.0f; // min shadow
	}
	res->_colorVBuf = rdi->createVertexBuffer(_vertCount * sizeof(Vec4f), res->_vertColorData);

	res->_16BitIndices = _16BitIndices;
	res->_geoObj = rdi->beginCreatingGeometry(Modules::renderer().getDefaultVertexLayout(DefaultVertexLayouts::ModelVertColor));

	// share original's data
	// can't free original or everything is broken
	rdi->setGeomVertexParams(res->_geoObj, _posVBuf, 0, 0, sizeof(Vec3f)); // loc 0 : vertPos
	rdi->setGeomVertexParams(res->_geoObj, _tanVBuf, 1, 0, sizeof(VertexDataTan)); // loc 1 : normal
	rdi->setGeomVertexParams(res->_geoObj, _tanVBuf, 2, sizeof(Vec3f), sizeof(VertexDataTan)); // loc 2 : tangent
	rdi->setGeomVertexParams(res->_geoObj, _staticVBuf, 3, 0, sizeof(VertexDataStatic)); // loc 3 : joints
																						 // loc 4 : weights
																						 // loc 5 : texCoords0
	// new elem (slot is unused actually ?)															// loc 6 : texCoords1
	rdi->setGeomVertexParams(res->_geoObj, res->_colorVBuf, 4, 0, sizeof(Vec4f)); // loc 7 : vertColorIn

	// also share indices
	rdi->setGeomIndexParams(res->_geoObj, _indexBuf, _16BitIndices ? IDXFMT_16 : IDXFMT_32);
...
void GeometryResource::release()
{
	...
	if (_colorVBuf != 0) {
		...
		// need geo object to be deleted first - on first call - by original resource
		// on the second call : free all the clones
		if (_geoObj != 0)
			return;
		// this is a clone + vertex color : only manages its own data
		if (_colorVBuf != defVertBuffer)
			rdi->destroyBuffer(_colorVBuf);
		delete[] _vertColorData;
		_vertColorData = 0x0;
	} else {
		if (_geoObj != 0)
...
bool GeometryResource::load(const char* data, int size)
{
	...
	if (version != 5)
		return raiseError("Unsupported version of geometry file");
	...
	for (uint32 i = 0; i < count; ++i) {
		...
		case 1: // Normal
			...
			for (uint32 j = 0; j < streamSize; ++j) {
				pData = elemcpy_le(&sh, (short*)(pData), 1);
				_vertTanData[j].normal.x = sh / 32767.0f;
				pData = elemcpy_le(&sh, (short*)(pData), 1);
				_vertTanData[j].normal.y = sh / 32767.0f;
				pData = elemcpy_le(&sh, (short*)(pData), 1);
				_vertTanData[j].normal.z = sh / 32767.0f;
			}
			break;
		case 2: // Tangent
			if (streamElemSize != 6) {
			...
			for (uint32 j = 0; j < streamSize; ++j) {
				pData = elemcpy_le(&sh, (short*)(pData), 1);
				_vertTanData[j].tangent.x = sh / 32767.0f;
				pData = elemcpy_le(&sh, (short*)(pData), 1);
				_vertTanData[j].tangent.y = sh / 32767.0f;
				pData = elemcpy_le(&sh, (short*)(pData), 1);
				_vertTanData[j].tangent.z = sh / 32767.0f;
			}
			break;
		case 3: // Bitangent
			if (streamElemSize != 6) {
			...
			for (uint32 j = 0; j < streamSize; ++j) {
				pData = elemcpy_le(&sh, (short*)(pData), 1);
				bitangents[j].x = sh / 32767.0f;
				pData = elemcpy_le(&sh, (short*)(pData), 1);
				bitangents[j].y = sh / 32767.0f;
				pData = elemcpy_le(&sh, (short*)(pData), 1);
				bitangents[j].z = sh / 32767.0f;
			}
			break;
		case 4: // Joint indices
			...
			for (uint32 j = 0; j < streamSize; ++j) {
				pData = elemcpy_le(&uc, (unsigned char*)(pData), 1);
				_vertStaticData[j].jointVec[0] = (float)uc;
				pData = elemcpy_le(&uc, (unsigned char*)(pData), 1);
				_vertStaticData[j].jointVec[1] = (float)uc;
				pData = elemcpy_le(&uc, (unsigned char*)(pData), 1);
				_vertStaticData[j].jointVec[2] = (float)uc;
				pData = elemcpy_le(&uc, (unsigned char*)(pData), 1);
				_vertStaticData[j].jointVec[3] = (float)uc;
			}
			break;
		case 5: // Weights
			if (streamElemSize != 4) {
			...
			for (uint32 j = 0; j < streamSize; ++j) {
				pData = elemcpy_le(&uc, (unsigned char*)(pData), 1);
				_vertStaticData[j].weightVec[0] = uc / 255.0f;
				pData = elemcpy_le(&uc, (unsigned char*)(pData), 1);
				_vertStaticData[j].weightVec[1] = uc / 255.0f;
				pData = elemcpy_le(&uc, (unsigned char*)(pData), 1);
				_vertStaticData[j].weightVec[2] = uc / 255.0f;
				pData = elemcpy_le(&uc, (unsigned char*)(pData), 1);
				_vertStaticData[j].weightVec[3] = uc / 255.0f;
			}
			break;
	...
	for (uint32 i = 0; i < numTargets; ++i) {
		...
		for (uint32 j = 0; j < count; ++j) {
			...
			case 0: // Position
				if (streamElemSize != 12)
					return raiseError("Invalid position morph stream");
			...
			case 1: // Normal
				if (streamElemSize != 12)
					return raiseError("Invalid normal morph stream");
				for (uint32 k = 0; k < morphStreamSize; ++k) {
			...
			case 2: // Tangent
				if (streamElemSize != 12)
					return raiseError("Invalid tangent morph stream");
				for (uint32 k = 0; k < morphStreamSize; ++k) {
			...
			case 3: // Bitangent
				if (streamElemSize != 12)
					return raiseError("Invalid bitangent morph stream");
	...
	for (uint32 i = 0; i < (uint32)_joints.size(); ++i) {
		...
		if (pos.x < _skelAABB.min.x)
			_skelAABB.min.x = pos.x;
		if (pos.y < _skelAABB.min.y)
			_skelAABB.min.y = pos.y;
		if (pos.z < _skelAABB.min.z)
			_skelAABB.min.z = pos.z;
		if (pos.x > _skelAABB.max.x)
			_skelAABB.max.x = pos.x;
		if (pos.y > _skelAABB.max.y)
			_skelAABB.max.y = pos.y;
		if (pos.z > _skelAABB.max.z)
			_skelAABB.max.z = pos.z;
	}
	...
	if (_vertCount > 0 && _indexCount > 0) {
		...
		// default vertex color data
		//		_vertColorData = new Vec4f[_vertCount];
		//		for (uint i = 0; i < _vertCount ; i++ ) {
		//			_vertColorData[i].x = 0.0f;// r
		//			_vertColorData[i].y = 0.0f;// g
		//			_vertColorData[i].z = 0.0f;// b
		//			_vertColorData[i].w = 0.0f;// min shadow
		//		}
		//		_colorVBuf = rdi->createVertexBuffer( _vertCount * sizeof( Vec4f ), _vertColorData );
		...
		//		rdi->setGeomVertexParams( _geoObj, _colorVBuf, 4, 0, sizeof( Vec4f ) );
	...
	if ((read || write) && mappedWriteStream == -1) {
		...
		case GeometryResData::GeometryElem:
			...
			case GeometryResData::GeoIndexStream:
				if (write)
					mappedWriteStream = GeometryResData::GeoIndexStream;
				return _indexData;
			case GeometryResData::GeoVertPosStream:
				if (write)
					mappedWriteStream = GeometryResData::GeoVertPosStream;
				return _vertPosData != 0x0 ? _vertPosData : 0x0;
			case GeometryResData::GeoVertTanStream:
				if (write)
					mappedWriteStream = GeometryResData::GeoVertTanStream;
				return _vertTanData != 0x0 ? _vertTanData : 0x0;
			case GeometryResData::GeoVertStaticStream:
				if (write)
					mappedWriteStream = GeometryResData::GeoVertStaticStream;
				return _vertStaticData != 0x0 ? _vertStaticData : 0x0;
			case GeometryResData::GeoVertColorSteam:
				if (write)
					mappedWriteStream = GeometryResData::GeoVertColorSteam;
				return _vertColorData != 0x0 ? _vertColorData : 0x0;
			}
		}
	}
...
void GeometryResource::unmapStream()
{
	if (mappedWriteStream >= 0) {
		...
		case GeometryResData::GeoVertStaticStream:
			...
			break;
		case GeometryResData::GeoVertColorSteam:
			if (_vertColorData != 0x0)
				rdi->updateBufferData(_geoObj, _colorVBuf, 0, _vertCount * sizeof(Vec4f), _vertColorData);
			break;
		}
